...
export const AuctionDetail: React.FC<AuctionDetailProps> = ({ auctionId, onNavigate }) => {
    ...
    const fetchAuctionData = async () => {
        try {
            ...
            setMessage({ type: "error", text: "Não foi possível carregar os detalhes do leilão." });
        } finally {
            setLoading(false);
        }
    };
    ...
    // Função de Dar Lance invocando a RPC place_bid do Supabase (Migração 004.sql)
    ...
    const minAllowed = (auction.current_price || vehicle.starting_price) + auction.min_increment;
                            ...
                            {auction.status === "ativo" ? (
                                <form onSubmit={handlePlaceBid} className="space-y-4">
                                    ...
                                    <button
                                        type="submit"
                                        disabled={bidding}
                                        className="w-full py-4 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-black font-extrabold uppercase tracking-wider text-sm transition-all shadow-lg shadow-emerald-500/20 disabled:opacity-50"
                                    >
